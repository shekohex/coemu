...
fn check_wasm_toolchain_installed(cargo_command: CargoCommand) -> Result<CargoCommandVersioned, String> {
    ...
    let prepare_command = |subcommand| {
        ...
        // Make sure the host's flags aren't used here, e.g. if an alternative
        // linker is specified in the RUSTFLAGS then the check we do
        // here will break unless we clear these.
