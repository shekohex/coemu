...
///
/// The builder itself is separated into multiple structs to make the setup type
/// safe.
///
/// Building a wasm binary:
///
/// 1. Call [`WasmBuilder::selector`] to create a new builder.
/// 2. Select the project to build using the methods of
///    [`WasmBuilderSelectProject`].
/// 3. Set additional `RUST_FLAGS` or a different name for the file containing
///    the WASM code using methods of [`WasmBuilder`].
...
impl WasmBuilder {
    ...
    pub fn selector() -> WasmBuilderSelectProject {
    ...
    ///
    /// If this function is not called, `file_name` defaults to `wasm.rs`
    ...
    ///
    /// `feature` needs to be a valid feature that is defined in the project
    /// `Cargo.toml`.
    ...
    pub fn build(self) {
        ...
        // As last step we need to generate our `rerun-if-changed` stuff. If a
        // build fails, we don't want to spam the output!
        generate_rerun_if_changed_instructions();
    }
}

/// Generate the name of the skip build environment variable for the current
/// crate.
...
fn provide_dummy_wasm_binary_if_not_exist(file_path: &Path) {
    if !file_path.exists() {
        crate::write_file_if_changed(
            file_path,
            "pub const WASM_BINARY: Option<&'static str> = None;\
                pub const WASM_BINARY_BLOATY: Option<&'static str> = None;",
        );
    }
}
...
fn generate_rerun_if_changed_instructions() {
    // Make sure that the `build.rs` is called again if one of the following env
    // variables changes.
...
///
/// The current project is determined by using the `CARGO_MANIFEST_DIR`
/// environment variable.
///
/// `file_name` - The name + path of the file being generated. The file contains
/// the constant `WASM_BINARY`, which contains the built wasm binary.
///
/// `project_cargo_toml` - The path to the `Cargo.toml` of the project that
/// should be built.
///
/// `default_rustflags` - Default `RUSTFLAGS` that will always be set for the
/// build.
///
...
fn build_project(
    ...
    crate::write_file_if_changed(
        ...
        format!(
            r#"
                pub const WASM_BINARY: Option<&'static str> = Some("{wasm_binary}");
                pub const WASM_BINARY_BLOATY: Option<&'static str> = Some("{wasm_binary_bloaty}");
            "#,
