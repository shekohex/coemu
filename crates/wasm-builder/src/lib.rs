...
//!
//! The Wasm builder is a tool that integrates the process of building the WASM
//! binary of your project into the main `cargo` build process.
//!
//! ## Project setup
//!
...
//! ```
//!
//! This will include the generated Wasm binary path as two static variables
//! `WASM_BINARY` and `WASM_BINARY_BLOATY`. The former is a compact Wasm binary
//! and the latter is the Wasm binary as being generated by the compiler. Both
//! variables have `Option<&'static str>` as type.
//!
//! ### Feature
//!
//! Wasm builder supports to enable cargo features while building the Wasm
//! binary. By default it will enable all features in the wasm build that are
//! enabled for the native build. Besides that, wasm builder supports the
//! special `runtime-wasm` feature. This `runtime-wasm` feature will be enabled
//! by the wasm builder when it compiles the Wasm binary. If this feature is not
//! present, it will not be enabled.
//!
//! ## Environment variables
//!
//! By using environment variables, you can configure which Wasm binaries are
//! built and how:
//!
//! - `SKIP_WASM_BUILD` - Skips building any Wasm binary. This is useful when
//!   only native should be recompiled. If this is the first run and there
//!   doesn't exist a Wasm binary, this will set both variables to `None`.
//! - `WASM_BUILD_TYPE` - Sets the build type for building Wasm binaries.
//!   Supported values are `wasm` or `wasmdebug`. By default the build type is
//!   equal to the build type used by the main build.
//! - `FORCE_WASM_BUILD` - Can be set to force a Wasm build. On subsequent calls
//!   the value of the variable needs to change. As wasm-builder instructs
//!   `cargo` to watch for file changes this environment variable should only be
//!   required in certain circumstances.
//! - `WASM_BUILD_RUSTFLAGS` - Extend `RUSTFLAGS` given to `cargo build` while
//!   building the wasm binary.
//! - `WASM_BUILD_NO_COLOR` - Disable color output of the wasm build.
//! - `WASM_TARGET_DIRECTORY` - Will copy any build Wasm binary to the given
//!   directory. The path needs to be absolute.
//! - `WASM_BUILD_TOOLCHAIN` - The toolchain that should be used to build the
//!   Wasm binaries. The format needs to be the same as used by cargo, e.g.
//!   `nightly-2020-02-20`.
//! - `WASM_BUILD_WORKSPACE_HINT` - Hint the workspace that is being built. This
//!   is normally not required as we walk up from the target directory until we
//!   find a `Cargo.toml`. If the target directory is changed for the build,
//!   this environment variable can be used to point to the actual workspace.
//! - `WASM_BUILD_STD` - Sets whether the Rust's standard library crates will
//!   also be built. This is necessary to make sure the standard library crates
//!   only use the exact WASM feature set that our executor supports. Enabled by
//!   default.
//! - `CARGO_NET_OFFLINE` - If `true`, `--offline` will be passed to all
//!   processes launched to prevent network access. Useful in offline
//!   environments.
//!
//! Each project can be skipped individually by using the environment variable
//! `SKIP_PROJECT_NAME_WASM_BUILD`. Where `PROJECT_NAME` needs to be replaced by
//! the name of the cargo project, e.g. `kitchensink-runtime` will be
//! `NODE_RUNTIME`.
//!
//! ## Prerequisites:
//!
...
//! or
//!
//! - rust stable and version at least 1.68.0 + `wasm32-unknown-unknown`
//!   toolchain
//!
...
//! 20.02.2020 using `rustup install nightly-2020-02-20`, the wasm target needs
//! to be installed as well `rustup target add wasm32-unknown-unknown
//! --toolchain nightly-2020-02-20`.
...
/// binary. Expects "debug", "release" or "production" as value.
///
/// When unset the WASM binary uses the same build type as the main cargo build
/// with the exception of a debug build: In this case the wasm build defaults to
/// `release` in order to avoid a slowdown when not explicitly requested.
...
fn get_cargo_command() -> CargoCommand {
    ...
    if let Some(cmd) = wasm_toolchain.map(|t| CargoCommand::new_with_args("rustup", &["run", &t, "cargo"])) {
        ...
        // If no command before provided us with a cargo that supports our
        // Substrate wasm env, we try to search one with rustup. If that
        // fails as well, we return the default cargo and let
...
fn get_rustup_command() -> Option<CargoCommand> {
    ...
    // Sort by the parsed version to get the latest version (greatest version)
...
impl CargoCommand {
    ...
    /// environment.
    ///
    /// This means that either the cargo version is at minimum 1.68.0 or this is
    /// a nightly cargo.
    ///
    /// Assumes that cargo version matches the rustc version.
    fn supports_substrate_wasm_env(&self) -> bool {
        // `RUSTC_BOOTSTRAP` tells a stable compiler to behave like a nightly.
        // So, when this env variable is set, we can assume that
        // whatever rust compiler we have, it is a nightly compiler. For
        // "more" information, see: https://github.com/rust-lang/rust/blob/fa0f7d0080d8e7e9eb20aa9cbf8013f96c81287f/src/libsyntax/feature_gate/check.rs#L891
...
/// Fetches a boolean environment variable. Will exit the process if the value
/// is invalid.
...
/// Returns whether we need to also compile the standard library when compiling
/// the runtime.
fn build_std_required() -> bool {
    crate::get_bool_environment_variable(crate::WASM_BUILD_STD).unwrap_or(false)
}
