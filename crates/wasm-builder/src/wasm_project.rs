...
impl WasmBinaryBloaty {
    ...
    pub fn bloaty_path(&self) -> &Path { &self.0 }
}
...
impl WasmBinary {
    ...
    pub fn wasm_binary_path(&self) -> &Path { &self.0 }
...
fn crate_metadata(cargo_manifest: &Path) -> Metadata {
    ...
    // If we can find a `Cargo.lock`, we assume that this is the workspace root
    // and there exists a `Cargo.toml` that we can use for getting the
    // metadata.
    let cargo_manifest =
        if let Some(mut cargo_lock) = find_cargo_lock(cargo_manifest) {
...
pub(crate) fn create_and_compile(
    ...
    let bloaty_blob_out_name = bloaty_blob_out_name_override
        .unwrap_or_else(|| bloaty_blob_default_name.clone());
    ...
    // Process the bloaty blob
    let bloaty_blob_binary = process_bloaty_blob(bloaty_blob_binary);

    // Try to compact and _compress_ the bloaty blob, if the *outer* profile
    // wants it.
    //
    // This is because, by default the inner profile will be set to `Release`
    // even when the outer profile is `Debug`, because the blob built in
    // `Debug` profile is too slow for normal development activities.
    let (compact_blob_path, compact_compressed_blob_path) =
        if build_config.outer_build_profile.wants_compact() {
            ...
            // TODO: Compress the compacted blob.
            (compact_blob_path, None)
        } else {
            (None, None)
        };

    if let Some(wasm_binary) = compact_blob_path.as_ref() {
        copy_blob_to_target_directory(project_cargo_toml, wasm_binary)
    }

    if let Some(wasm_binary_compressed) = compact_compressed_blob_path.as_ref()
    {
    ...
    if let Err(err) =
        adjust_mtime(&bloaty_blob_binary, final_blob_binary.as_ref())
    {
...
///
/// We add the bloaty and the compressed/compact wasm file to the
/// `rerun-if-changed` files. Cargo/Rustc determines based on the timestamp of
/// the `invoked.timestamp` file that can be found in the `OUT_DIR/..`, if it
/// needs to rerun a `build.rs` script. The problem is that this
/// `invoked.timestamp` is created when the `build.rs` is executed and the wasm
/// binaries are created later. This leads to them having a later mtime than the
/// `invoked.timestamp` file and thus, cargo/rustc always re-executes the
/// `build.rs` script. To hack around this, we copy the mtime of the `invoked.
/// timestamp` to the wasm binaries.
fn adjust_mtime(
...
fn get_crate_name(cargo_manifest: &Path) -> String {
    let cargo_toml: Table = toml::from_str(
        &fs::read_to_string(cargo_manifest)
            .expect("File exists as checked before; qed"),
    )
...
fn get_wasm_workspace_root() -> PathBuf {
    ...
    loop {
        match out_dir.parent() {
            Some(parent) if out_dir.ends_with("build") => {
...
fn create_project_cargo_toml(
    ...
    {
        // Iterate over all patches and make the patch path absolute from the
        // workspace root path.
        patch
            .iter_mut()
            .filter_map(|p| {
    ...
    write_file_if_changed(
        ...
        toml::to_string_pretty(&wasm_workspace_toml)
            .expect("Wasm workspace toml is valid; qed"),
    );
}

/// Find a package by the given `manifest_path` in the metadata. In case it
/// can't be found by its manifest_path, fallback to finding it by name; this is
/// necessary during publish because the package's manifest path will be
/// *generated* within a specific packaging directory, thus it won't be found by
/// its original path anymore.
///
...
fn find_package_by_manifest_path<'a>(
    ...
    if let Some(pkg) = pkgs_by_name.first() {
        ...
        panic!(
            "Failed to find entry for package {pkg_name} ({manifest_path:?})."
        );
    }
}
...
fn project_enabled_features(
    ...
    let package =
        find_package_by_manifest_path(pkg_name, cargo_manifest, crate_metadata);
    ...
    let mut enabled_features = package
        ...
        .filter(|(f, v)| {
            ...
            // If this is a feature that corresponds only to an optional
            // dependency and this feature is enabled by the `std`
            // feature, we assume that this is only done through the
            // `std` feature. This is a bad heuristic and should
            // be removed after namespaced features are landed:
            // https://doc.rust-lang.org/cargo/reference/unstable.html#namespaced-features
            // Then we can just express this directly in the `Cargo.toml` and do
            // not require this heuristic anymore. However, for the
            // transition phase between now and namespaced
            // features already being present in nightly, we need this code to
            ...
            // We don't want to enable the `std`/`default` feature for the wasm
            // build and we need to check if the feature is enabled
            // by checking the env variable.
            *f != "std"
                && *f != "default"
...
fn create_project(
    ...
    let mut enabled_features = project_enabled_features(
        &crate_name,
        project_cargo_toml,
        crate_metadata,
    );

    if has_runtime_wasm_feature_declared(
        &crate_name,
        project_cargo_toml,
        crate_metadata,
    ) {
    ...
    let mut enabled_features =
        enabled_features.into_iter().collect::<HashSet<_>>();
    ...
    if let Some(crate_lock_file) = find_cargo_lock(project_cargo_toml) {
        ...
        crate::copy_file_if_changed(
            crate_lock_file,
            wasm_project_folder.join("Cargo.lock"),
        );
    }
...
impl Profile {
    ...
    /// # Note
    ///
    /// Usually this is the same as [`Self::name`] with the exception of the
    /// debug profile which is called `dev`.
    ...
    fn wants_compact(&self) -> bool { !matches!(self, Self::Debug) }
}
...
impl BuildConfiguration {
    /// Create a [`BuildConfiguration`] by detecting which profile is used for
    /// the main build and checking any env var overrides.
    ///
    /// We cannot easily determine the profile that is used by the main cargo
    /// invocation because the `PROFILE` environment variable won't contain
    /// any custom profiles like "production". It would only contain the
    /// builtin profile where the custom profile inherits from. This is why
    /// we inspect the build path to learn which profile is used.
    ///
    /// When not overriden by a env variable we always default to building wasm
    /// with the `Release` profile even when the main build uses the debug
    /// build. This is because wasm built with the `Debug` profile is too
    /// slow for normal development activities and almost never intended.
    ///
    /// When cargo is building in `--profile dev`, user likely intends to
    /// compile fast, so we don't bother producing compact or compressed
    /// blobs.
    ///
    /// # Note
    ///
    ...
    fn detect(wasm_project: &Path) -> Self {
        let (name, overriden) = if let Ok(name) =
            env::var(crate::WASM_BUILD_TYPE_ENV)
        {
            (name, true)
        } else {
            // First go backwards to the beginning of the target directory.
            // Then go forwards to find the "wbuild" directory.
            // We need to go backwards first because when starting from the
            // root there might be a chance that someone has
            // a "wbuild" directory somewhere in the path.
        ...
        let outer_build_profile =
            Profile::iter().find(|p| p.directory() == name);
        let blob_build_profile = match (outer_build_profile.clone(), overriden)
        {
            // When not overriden by a env variable we default to using the
            // `Release` profile for the wasm build even when the
            // main build uses the debug build. This is because the
            // `Debug` profile is too slow for normal development activities.
            ...
            // This allows us to continue building when a custom profile is used
            // for the main builds cargo. When explicitly passing a
            // profile via env variable we are not doing a fallback.
            (None, false) => {
            ...
            (None, true) => {
                // We use println! + exit instead of a panic in order to have a
                // cleaner output.
                println!(
        ...
        BuildConfiguration {
            outer_build_profile: outer_build_profile
                .unwrap_or(Profile::Release),
            blob_build_profile,
        }
    }
}
...
fn offline_build() -> bool { env::var(OFFLINE).map_or(false, |v| v == "true") }
...
fn build_bloaty_blob(
    ...
    // nowadays when compiling WASM the Rust compiler has more features
    // enabled by default.
    //
    ...
    //
    // So here we force the compiler to also compile the standard library crates
    // for us to make sure that they also only use the MVP features.
    if crate::build_std_required() {
        // Unfortunately this is still a nightly-only flag, but FWIW it is
        // pretty widely used so it's unlikely to break without a
        // replacement.
...
fn copy_bloaty_blob(
    ...
    fs::copy(in_path, &bloaty_path)
        .expect("Copying the bloaty file to the project dir.");
    WasmBinaryBloaty(bloaty_path)
}

fn process_bloaty_blob(
    bloty_blob_binary: WasmBinaryBloaty,
) -> WasmBinaryBloaty {
    let processor = externref::processor::Processor::default();
    let wasm_bytes = fs::read(bloty_blob_binary.bloaty_path())
        .expect("Reading the bloaty file.");
    let processed_wasm_bytes = processor
        .process_bytes(&wasm_bytes)
        .expect("Processing the bloaty file.");
    fs::write(bloty_blob_binary.bloaty_path(), processed_wasm_bytes)
        .expect("Writing the bloaty file.");
    bloty_blob_binary
}
...
impl<'a> Hash for DeduplicatePackage<'a> {
    fn hash<H: Hasher>(&self, state: &mut H) { self.identifier.hash(state); }
}

impl<'a> PartialEq for DeduplicatePackage<'a> {
    fn eq(&self, other: &Self) -> bool { self.identifier == other.identifier }
}
...
impl<'a> Deref for DeduplicatePackage<'a> {
    ...
    fn deref(&self) -> &Self::Target { self.package }
}
...
fn generate_rerun_if_changed_instructions(
    ...
    while let Some(dependency) = dependencies.pop() {
        ...
        let package = metadata
            ...
            .find(|p| {
                // Check that the name matches and that the version matches or
                // this is a git or path dep. A git or path
                // dependency can only occur once, so we don't
                // need to check the version.
                (path_or_git_dep || dependency.req.matches(&p.version))
                    && dependency.name == p.name
            });
    ...
    // Make sure that if any file/folder of a dependency change, we need to
    // rerun the `build.rs`
...
fn package_rerun_if_changed(package: &DeduplicatePackage) {
    ...
    WalkDir::new(&manifest_path)
        .into_iter()
        .filter_entry(|p| {
            // Ignore this entry if it is a directory that contains a
            // `Cargo.toml` that is not the `Cargo.toml` related to
            // the current package. This is done to ignore sub-crates of a
            // crate. If such a sub-crate is a dependency, it will be processed
...
/// Copy the blob binary to the target directory set in `WASM_TARGET_DIRECTORY`
/// environment variable. If the variable is not set, this is a no-op.
fn copy_blob_to_target_directory(
    cargo_manifest: &Path,
    blob_binary: &WasmBinary,
) {
    ...
    if !target_dir.is_absolute() {
        // We use println! + exit instead of a panic in order to have a cleaner
        // output.
        println!(
